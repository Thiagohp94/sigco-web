...
import { Plus, TrendingUp, Edit2, Trash2, Search, X, User, Calendar, AlertTriangle, Package, ChevronDown } from "lucide-react";
...
const METHOD_LABELS: Record<string, string> = {
  ...
  debit_card: "Cartao de Debito", bank_transfer: "Transferencia",
  insurance: "Convenio", check: "Cheque",
};
...
interface ProcedureItem { id: string; name: string; price: string; }
type Filter = "all" | "pending" | "overdue" | "paid";

const EMPTY_META = { due_date: "", installments: "1", billing_day: "", payment_method: "", notes: "", already_paid: "false" };
...
export default function ContasReceberPage() {
  ...
  const { data: installmentsData } = useQuery<any[]>({
    queryKey: ["installments", expandedId],
    queryFn: () => api.get("/financial/installments", { params: { receivable_id: expandedId } }).then((r) => r.data),
    enabled: !!expandedId,
  });

  const payInstallmentMutation = useMutation({
    mutationFn: ({ instId, method }: { instId: string; method: string }) =>
      api.post(`/financial/installments/${instId}/pay`, null, {
        params: { amount_paid: installmentsData?.find((i: any) => i.id === instId)?.amount, method }
      }).then((r) => r.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["installments", expandedId] });
      qc.invalidateQueries({ queryKey: ["receivables"] });
      qc.invalidateQueries({ queryKey: ["financial-summary"] });
      toast.success("Parcela registrada!");
    },
    onError: () => toast.error("Erro ao registrar parcela"),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      const payload: any = {
        ...
        description: descriptionAuto || "Procedimento odontologico",
        billing_day: meta.billing_day ? parseInt(meta.billing_day) : undefined,
        amount: total,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["receivables"] });
      qc.invalidateQueries({ queryKey: ["financial-summary"] });
      toast.success(editingId ? "Cobranca atualizada!" : "Cobranca criada!");
      closeForm();
    },
    onError: (e: any) => toast.error(apiErrorMessage(e, "Erro ao salvar cobranca")),
  });
  ...
  function onSelectAppointment(appt: any) {
    ...
    if (appt?.procedure_name) {
      setItems([newItem(appt.procedure_name, appt.procedure_price ? String(appt.procedure_price) : "")]);
    }
    if (appt?.start_time) {
  ...
  function addFromProcedure(proc: any) { setItems(prev => [...prev, newItem(proc.name, proc.price ? String(proc.price) : "")]); }
  ...
  function openEdit(r: any) {
    ...
    setMeta({ due_date: r.due_date, installments: String(r.installments), billing_day: r.billing_day ? String(r.billing_day) : "", payment_method: r.payment_method ?? "", notes: r.notes ?? "", already_paid: "false" });
    setShowForm(true);
  }

  function closeForm() {
    setShowForm(false); setEditingId(null); setMeta({ ...EMPTY_META });
    setItems([newItem()]); setSelectedPatient(null); setPatientSearch(""); setSelectedAppointment(null);
  }
  ...
  const enriched = receivables.map((r) => ({
    ...r, _overdue: isOverdue(r),
    _status: isOverdue(r) && r.status !== "paid" ? "overdue" : r.status,
  }));
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {enriched.map((r) => {
            const cfg = STATUS_LABELS[r._status] ?? STATUS_LABELS.pending;
            return (
              <div key={r.id} className={cn(cardBase, r._overdue && r.status !== "paid" && (isDark ? "border-red-500/20" : "border-red-200"))}>
                <div className="flex items-center gap-4 flex-wrap">
                  <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold text-white shrink-0",
                    r._overdue && r.status !== "paid" ? "bg-gradient-to-br from-red-500 to-rose-600 shadow-lg shadow-red-500/20" : "bg-gradient-to-br from-cyan-500 to-cyan-600 shadow-lg shadow-cyan-500/20")}>
                    {r._overdue && r.status !== "paid" ? <AlertTriangle className="w-4 h-4" /> : (r.patient_name?.slice(0, 2).toUpperCase() ?? "?")}
                  </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className={cn("text-xs font-medium px-2.5 py-1 rounded-full border hidden sm:block", isDark ? cfg.dark : cfg.light)}>{cfg.label}</span>
                        ...
                        <button
                          ...
                          className={cn("px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all",
                            expandedId === r.id
                              ...
                              : isDark ? "bg-white/5 text-white/40 hover:text-violet-300" : "bg-gray-50 text-gray-500 hover:text-violet-600")}>
                          {r.installments}x <ChevronDown className={cn("w-3 h-3 inline transition-transform", expandedId === r.id && "rotate-180")} />
                        </button>
                      )}
                      {(r._status === "pending" || r._status === "overdue" || r._status === "partial") && r.installments <= 1 && (
              ...
              {expandedId === r.id && (
                <div className={cn("mt-3 pt-3 border-t space-y-1.5", isDark ? "border-white/5" : "border-gray-100")}>
                  <p className={cn("text-xs font-semibold mb-2", isDark ? "text-white/40" : "text-gray-400")}>
                    Cronograma de parcelas
                  </p>
                  {(installmentsData ?? []).map((inst: any) => {
                    const instCfg = STATUS_LABELS[inst.status] ?? STATUS_LABELS.pending;
                    const instOverdue = inst.status !== "paid" && isPast(startOfDay(parseISO(inst.due_date + "T00:00:00")));
                    return (
                      <div key={inst.id} className={cn(
                        "flex items-center justify-between rounded-xl px-3 py-2.5 border transition-all",
                        instOverdue ? isDark ? "border-red-500/20 bg-red-500/5" : "border-red-200 bg-red-50"
                        ...
                        <div className="flex items-center gap-3">
                          <span className={cn("w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0",
                            inst.status === "paid" ? "bg-emerald-500 text-white" : instOverdue ? "bg-red-500 text-white" : isDark ? "bg-white/10 text-white/50" : "bg-gray-200 text-gray-500")}>
                            {inst.number}
                          </span>
                          <div>
                            <p className={cn("text-sm font-medium", isDark ? "text-white/80" : "text-gray-700")}>
                              {format(parseISO(inst.due_date + "T00:00:00"), "dd/MM/yyyy")}
                              {instOverdue && <span className="ml-1.5 text-red-400 text-xs">VENCIDA</span>}
                            </p>
                            {inst.paid_at && <p className={cn("text-xs", isDark ? "text-emerald-400/60" : "text-emerald-600")}>
                              Pago em {format(parseISO(inst.paid_at), "dd/MM/yyyy")}
                            </p>}
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={cn("text-sm font-bold", isDark ? "text-white" : "text-gray-900")}>{fmt(inst.amount)}</span>
                          <span className={cn("text-xs px-2 py-0.5 rounded-full border hidden sm:block", isDark ? instCfg.dark : instCfg.light)}>
                            {instCfg.label}
                          </span>
                          {inst.status !== "paid" && (
                            <button
                              onClick={() => payInstallmentMutation.mutate({ instId: inst.id, method: "pix" })}
                  ...
                  {!installmentsData && <p className={cn("text-xs text-center py-2", isDark ? "text-white/20" : "text-gray-400")}>Carregando parcelas...</p>}
                </div>
              )}
            );
          })}
      ...
      <Dialog open={showForm} onOpenChange={closeForm}>
        <DialogContent className={cn("max-w-xl rounded-2xl max-h-[90vh] overflow-y-auto", isDark ? "glass-strong border-white/10 text-white" : "bg-white border-gray-200")}>
          ...
          <div className="space-y-5 pb-2">
            ...
            {selectedPatient && (
              <div className="space-y-1.5">
                <Label className={labelCls}>Atendimento <span className={cn("text-xs", txtMuted)}>(opcional — preenche valores automaticamente)</span></Label>
                {selectedAppointment ? (
                  <div className={cn("flex items-center justify-between rounded-xl px-3 py-2.5 border text-sm", isDark ? "bg-violet-500/10 border-violet-500/20" : "bg-violet-50 border-violet-200")}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-sm", isDark ? "text-violet-300" : "text-violet-700")}>
                        {selectedAppointment.start_time ? format(parseISO(selectedAppointment.start_time), "dd/MM/yyyy HH:mm") : "Atendimento"}
                        {selectedAppointment.procedure_name && ` — ${selectedAppointment.procedure_name}`}
                      </span>
                    </div>
                    <button onClick={() => setSelectedAppointment(null)}><X className={cn("w-4 h-4", isDark ? "text-violet-400/40" : "text-violet-400")} /></button>
                  </div>
                ) : (
                  <select className="w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none" style={selectStyle}
                    onChange={(e) => { const a = appointmentResults.find((x: any) => x.id === e.target.value); if (a) onSelectAppointment(a); }} defaultValue="">
            ...
            <div className="space-y-2">
                ...
                {allProcedures.length > 0 && (
                  <div className="relative">
                    <button
                      ...
                      className={cn("text-xs flex items-center gap-1 px-2.5 py-1.5 rounded-lg border transition-all",
                        showCatalog
                          ? isDark ? "bg-cyan-500/20 border-cyan-500/30 text-cyan-300" : "bg-cyan-500 border-cyan-500 text-white"
                          : isDark ? "border-white/10 text-cyan-400/70 hover:bg-cyan-500/10 hover:text-cyan-300" : "border-gray-200 text-cyan-600 hover:bg-cyan-50")}>
                      <Package className="w-3 h-3" /> + do catálogo <ChevronDown className={cn("w-3 h-3 transition-transform", showCatalog && "rotate-180")} />
                    </button>
                    {showCatalog && (
                      <div className={cn("absolute right-0 top-full mt-1 z-50 rounded-xl border shadow-xl min-w-[220px] max-h-56 overflow-y-auto",
                        isDark ? "glass-strong border-white/10" : "bg-white border-gray-200")}>
                        {allProcedures.filter((p: any) => p.is_active !== false).map((p: any) => (
                          <button key={p.id}
                            onClick={() => { addFromProcedure(p); setShowCatalog(false); }}
                            className={cn("w-full text-left px-3 py-2.5 text-sm flex justify-between items-center transition-colors border-b last:border-0",
                              isDark ? "hover:bg-white/5 border-white/5" : "hover:bg-gray-50 border-gray-100")}>
                            <span className={cn(isDark ? "text-white/80" : "text-gray-700")}>{p.name}</span>
                            <span className={cn("text-xs font-semibold ml-3 shrink-0", p.price ? (isDark ? "text-cyan-400" : "text-cyan-600") : (isDark ? "text-white/20" : "text-gray-300"))}>
                              {p.price ? fmt(p.price) : "Sem preço"}
                            </span>
                          </button>
                        ))}
                      </div>
              ...
              <div className="space-y-2">
                {items.map((item, idx) => (
                  <div key={item.id} className="flex gap-2 items-center">
                    <div className={cn("w-5 h-5 rounded-md flex items-center justify-center text-xs font-bold shrink-0", isDark ? "bg-white/5 text-white/30" : "bg-gray-100 text-gray-400")}>{idx + 1}</div>
                    <Input className={cn("flex-1", inputCls)} placeholder="Ex: Limpeza, Canal..." value={item.name} onChange={(e) => updateItem(item.id, "name", e.target.value)} />
                    <Input type="number" step="0.01" min="0" className={cn("w-28 shrink-0", inputCls)} placeholder="R$ 0,00" value={item.price} onChange={(e) => updateItem(item.id, "price", e.target.value)} />
                    <button onClick={() => removeItem(item.id)} disabled={items.length === 1}
                      className={cn("p-1.5 rounded-lg transition-all shrink-0", items.length === 1 ? "opacity-20 cursor-not-allowed" : "", isDark ? "text-white/20 hover:text-red-400 hover:bg-red-500/10" : "text-gray-300 hover:text-red-500 hover:bg-red-50")}>
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ))}
              ...
              {items.length > 1 && (
                <div className={cn("flex items-center justify-between rounded-xl px-4 py-2.5 border", isDark ? "bg-white/3 border-white/5" : "bg-gray-50 border-gray-200")}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className={labelCls}>Vencimento 1ª parcela *</Label>
            ...
            {parseInt(meta.installments) > 1 && (
              <div className={cn("rounded-xl border p-3 space-y-2", isDark ? "bg-violet-500/5 border-violet-500/15" : "bg-violet-50 border-violet-200")}>
                <p className={cn("text-xs font-medium", isDark ? "text-violet-300" : "text-violet-700")}>
                  Parcelamento em {meta.installments}x de {fmt(total / parseInt(meta.installments || "1"))}
                </p>
                <div className="space-y-1.5">
                  <Label className={cn("text-xs", isDark ? "text-violet-300/70" : "text-violet-600")}>Dia de cobrança das demais parcelas</Label>
                  <div className="flex items-center gap-2">
                    <Input
                      type="number" min={1} max={28}
                      value={meta.billing_day}
                      onChange={(e) => setM("billing_day", e.target.value)}
                      placeholder="Ex: 5 (dia 5 de cada mês)"
                      className={cn("flex-1", inputCls)}
                    />
                    <span className={cn("text-xs shrink-0", isDark ? "text-violet-300/50" : "text-violet-500")}>dia/mês (1–28)</span>
                  </div>
                  <p className={cn("text-xs", isDark ? "text-violet-300/40" : "text-violet-400")}>
                    Se não informado, usa o mesmo dia do vencimento da 1ª parcela.
                  </p>
                </div>
              </div>
            )}
            ...
            {!editingId && (
              <div className={cn("flex items-center gap-3 rounded-xl p-3 border cursor-pointer select-none",
                ...
                <input type="checkbox" readOnly checked={meta.already_paid === "true"} className="w-4 h-4 accent-emerald-500 cursor-pointer" />
                <span className={cn("text-sm font-medium", isDark ? "text-emerald-300" : "text-emerald-700")}>
                  Ja foi pago no momento do atendimento
                </span>
              </div>
            )}
            ...
            <div className="flex gap-2 justify-end pt-1">
              ...
              <Button disabled={saveMutation.isPending || !selectedPatient || !meta.due_date || total <= 0}
                onClick={() => saveMutation.mutate()}
                className="bg-gradient-to-r from-cyan-500 to-cyan-600 border-0 rounded-xl text-white">
                {saveMutation.isPending ? "Salvando..." : editingId ? "Salvar" : `Criar cobranca · ${fmt(total)}`}
              </Button>
            </div>
          </div>
        </DialogContent>
